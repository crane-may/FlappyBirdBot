...
def append_pipe(frame, frame_delay, now):
  (new_pipe_h, new_pipe_t) = detect.pipe_h(frame, PIPE_SPACE, DETECT_PIPE_Y, DETECT_PIPE_TOP_X, GROUND_X, PIPE_W, PIPE_V, frame_delay)
...
def correct_b_h(frame, frame_delay, now):
  ...
  detect_b_h = detect.bird_h(frame, DETECT_BIRD_Y, GROUND_X, PIPE_SPACE)
...
def detect_thread():
  ...
  while(True):
    ...
    frame_delay = detect.timer(frame, DETECT_TIMER_X) + TAP_DELAY
    if frame_delay > TAP_DELAY and frame_delay < 0.25:
      append_pipe(frame, frame_delay, now)
      correct_b_h(frame, frame_delay, now)
    
      filename = '%f.jpg'%now
      log_w(now-frame_delay, "[B]<%s>"%filename)
      
      # cv2.imwrite('datalog/%s'%filename,frame)
    else:
